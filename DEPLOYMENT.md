...
- האתר חי בכתובת `https://<הדומיין-שלכם>` (לדוגמה `https://fun-israel.co.il`).
...
> אם השם תפוס, בדקו וריאציות: `funisrael.co.il`, `fun-israel.co.il`, `funisrael-rentals.co.il`.
...
כשהדומיין שלכם (לדוגמה `fun-israel.co.il`):

1. ב-Vercel: היכנסו לפרויקט → **Settings → Domains → Add**.
2. הזינו את הדומיין (לדוגמה `fun-israel.co.il`) → **Add**.
...
```
NEXT_PUBLIC_SITE_URL = https://fun-israel.co.il
```
...
כדי שהמייל יראה לפונה כאילו הוא מ-`hello@fun-israel.co.il` ולא מ-
`onboarding@resend.dev`:

1. ב-Resend → **Domains → Add Domain** → הזינו `fun-israel.co.il`.
...
5. ב-Vercel → Environment Variables → עדכנו:
   ...
   ```
   RESERVATION_FROM_EMAIL = "FUN-ISRAEL <hello@fun-israel.co.il>"
   ```
...
> 📥 גם הקופסה `hello@fun-israel.co.il` אצלכם — תצטרכו להגדיר
> forwarding אליה. ב-LiveDNS/Cloudflare/וכו' יש "Email Forwarding"
> חינם — הגדירו `hello@fun-israel.co.il` → `zehavialon4@gmail.com`.
...
- [ ] `https://fun-israel.co.il` עולה עם תעודת SSL חוקית (מנעול בכרום).
- [ ] `https://www.fun-israel.co.il` מפנה ל-`https://fun-israel.co.il`.
...
- [ ] `https://fun-israel.co.il/sitemap.xml` עובד.
- [ ] `https://fun-israel.co.il/robots.txt` עובד.
...
- [ ] `https://fun-israel.co.il/admin` דורש סיסמה.
